...
def get_available_tools():
    ...
    return [
        ...
        },
        {
            ...
            "description": "Get the recipients of a DocuSign envelope",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "envelope_id": {
...
async def call_tool(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if tool_name == "getenvelope":
        ...
        elif tool_name == "get_envelope_recipients":
            envelope_id = args.get("envelope_id")
            if not envelope_id:
                return {"success": False, "error": "envelope_id is required"}
            ...
            # Try to use real DocuSign API
            try:
                from esign_docusign import get_envelope_recipients_docusign
                result = get_envelope_recipients_docusign(envelope_id)
                logger.info(f"🔍 POKE DEBUG: Real DocuSign recipients result: {result}")
                return result
            except ImportError as e:
                logger.warning(f"⚠️ DocuSign not available: {e}, using mock response")
                return {
                    ...
                    "envelope_id": envelope_id,
                    "recipients": [],
                    "message": "Mock recipients - DocuSign integration not available"
                }
